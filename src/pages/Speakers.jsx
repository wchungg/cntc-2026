...
function Tag({ label }) {
  ...
  return <span className={`${base} ${styles[label] || "bg-gray-100 text-gray-700"}`}>{label}</span>
}

function SpeakerCard({ img, name, title, tags }) {
  return (
    <div className="w-[315px] bg-white ring-1 ring-black/10 shadow-[4px_4px_10px_0px_rgba(0,0,0,0.25)]">
      ...
      <div className="px-4 py-4 h-[136px] bg-[#F5F6F7]">
        <div className="text-[26px] font-semibold text-[#000000]">{name}</div>
        <div className="mt-2 text-[17px] leading-4 text-[#6B6B6B]">{title}</div>
...
export const Speakers = () => {
  return <div className='min-h-screen bg-white'>
    ...
    <header className='relative h-[680px] w-full'>
      <img src={speakersHero} alt="Speakers" className='h-full w-full object-cover' />
          ...
          <div className="mx-auto w-full max-w-7xl mt-90">
            <div className="ml-auto max-w-md text-right">
              ...
              <p className="text-[20px] leading-5 text-white/85">
                ...
                neuroscience in academia and industry. <br />
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="grid items-center gap-10 md:grid-cols-[360px_1fr]">
        ...
        <div className="flex gap-14 justify-start">
          {keystoneSpeakers.map((s) => (
            <SpeakerCard key={s.name} img={s.img} name={s.name} title={s.title} />
          ))}
        </div>
      </section>
    </div>
...
<div className="mx-auto w-full max-w-7xl mt-20">
  <section className="grid items-start gap-12 md:grid-cols-[220px_1fr]">
    ...
    <div className="grid gap-y-10 gap-x-32 sm:grid-cols-2 lg:grid-cols-3">
      {panelists.map((p) => (
        <SpeakerCard
          key={p.name + p.img}
          img={p.img}
